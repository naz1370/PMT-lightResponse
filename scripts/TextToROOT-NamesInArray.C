...
int readInInten() {
    ...
    for (int fileNumber = 0; fileNumber < maxFileNumber; ++fileNumber) {
        ...
        TH1D* histogram2 = new TH1D(("PhotonCount_" + std::to_string(fileNumber)).c_str(),
                                    ("Photon Count " + std::to_string(fileNumber)).c_str(),
                                    nBins, xMin, xMax);
        
        TH1D* histogram3 = new TH1D(("scopeCount_" + std::to_string(fileNumber)).c_str(),
                                    ("Sample Count " + std::to_string(fileNumber)).c_str(),
                                    nBins, xMin, xMax);
