...
int readInInten() {
    const int maxFileNumber = 18;  // Change this to the number of files you have
    // const int maxRoundNumber=10;
    // Number of bins
    const double xMin = 200; // Minimum x value
    const double xMax = 1100.53; // Maximum x value
    double binSize = 0.57;
    double NBinsCalc = (xMax - xMin) / binSize;
    const int nBins = NBinsCalc; // Number of bins

    // Create a pointer to store histogram2 of the first file

    // Loop over file numbers
    std::cout << "Looping over files" << std::endl;
    for (int fileNumber = 0; fileNumber < maxFileNumber; ++fileNumber) {
      //for(int roundNumber = 0; roundNumber < maxRoundNumber; ++roundNumber) {
       


 // Generate the file name
       std::string fileName = "13Sepetember_F24-1250_200usIntegrationTime_nmDG_3mmS_7-5Amp_Cable5_dial_";
       std::string filenameIndex = "-9Amps";
       int  dialNumbs = 520+20*fileNumber;
       std::string fileNumbStr = std::to_string(fileNumber) ;
       //std::string roundNumberStr = std::to_string(roundNumber);
       fileName=fileName + std::to_string(dialNumbs)+"_" + std::to_string(fileNumber)+ ".txt";
